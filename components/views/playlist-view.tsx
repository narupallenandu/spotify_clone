"use client"

import { Play, Pause, Heart, Clock, MoreHorizontal } from "lucide-react"
import { playlists, formatDuration } from "@/lib/music-data"
import { usePlayerStore, usePlayerActions } from "@/lib/player-store"
import { cn } from "@/lib/utils"

interface PlaylistViewProps {
  playlistId: string
}

export function PlaylistView({ playlistId }: PlaylistViewProps) {
  const playlist = playlists.find((p) => p.id === playlistId)
  const player = usePlayerStore()
  const actions = usePlayerActions()

  if (!playlist) {
    return (
      <div className="flex items-center justify-center h-full">
        <p className="text-muted-foreground">Playlist not found</p>
      </div>
    )
  }

  const isPlaylistPlaying =
    player.isPlaying &&
    player.currentTrack &&
    playlist.tracks.some((t) => t.id === player.currentTrack?.id)

  const totalDuration = playlist.tracks.reduce((sum, t) => sum + t.duration, 0)
  const totalMin = Math.floor(totalDuration / 60)

  return (
    <div className="flex flex-col">
      {/* Header */}
      <div className="flex flex-col sm:flex-row items-end gap-6 p-6 pb-8 bg-gradient-to-b from-accent/40 to-background">
        <img
          src={playlist.coverUrl}
          alt={playlist.name}
          className="size-48 lg:size-56 rounded-lg shadow-2xl object-cover flex-shrink-0"
        />
        <div className="flex flex-col gap-2">
          <span className="text-xs font-medium text-foreground uppercase tracking-wider">
            Playlist
          </span>
          <h1 className="text-4xl lg:text-6xl font-extrabold text-foreground text-balance leading-tight">
            {playlist.name}
          </h1>
          <p className="text-sm text-muted-foreground">{playlist.description}</p>
          <div className="flex items-center gap-1.5 text-sm text-muted-foreground">
            <span>
              {playlist.tracks.length} songs, {totalMin} min
            </span>
          </div>
        </div>
      </div>

      {/* Actions */}
      <div className="flex items-center gap-6 px-6 py-4">
        <button
          onClick={() =>
            isPlaylistPlaying
              ? actions.togglePlay()
              : actions.playAlbumOrPlaylist(playlist.tracks, 0)
          }
          className="flex size-12 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-lg transition-transform hover:scale-105"
          aria-label={isPlaylistPlaying ? "Pause" : "Play"}
        >
          {isPlaylistPlaying ? (
            <Pause className="size-5 fill-current" />
          ) : (
            <Play className="size-5 fill-current ml-0.5" />
          )}
        </button>
        <button className="p-2 text-muted-foreground hover:text-foreground transition-colors" aria-label="More options">
          <MoreHorizontal className="size-6" />
        </button>
      </div>

      {/* Track list */}
      <div className="px-6">
        <div className="flex items-center gap-4 px-3 py-2 border-b border-border text-xs text-muted-foreground uppercase tracking-wider">
          <span className="w-8 text-center">#</span>
          <span className="flex-1">Title</span>
          <span className="hidden sm:block w-40">Album</span>
          <Clock className="size-4" />
        </div>

        {playlist.tracks.map((track, idx) => {
          const isCurrentTrack = player.currentTrack?.id === track.id
          const isTrackPlaying = isCurrentTrack && player.isPlaying

          return (
            <button
              key={`${track.id}-${idx}`}
              onClick={() =>
                isCurrentTrack
                  ? actions.togglePlay()
                  : actions.playTrack(track, playlist.tracks, idx)
              }
              className={cn(
                "group flex w-full items-center gap-4 rounded-md px-3 py-2.5 transition-colors hover:bg-accent text-left",
                isCurrentTrack && "bg-accent/50"
              )}
            >
              <span className="w-8 text-center text-sm tabular-nums">
                {isTrackPlaying ? (
                  <span className="inline-flex gap-0.5 items-end h-3.5">
                    <span className="w-0.5 bg-primary animate-pulse h-2" />
                    <span className="w-0.5 bg-primary animate-pulse h-3 delay-75" />
                    <span className="w-0.5 bg-primary animate-pulse h-1.5 delay-150" />
                  </span>
                ) : (
                  <span className={cn("group-hover:hidden", isCurrentTrack ? "text-primary" : "text-muted-foreground")}>
                    {idx + 1}
                  </span>
                )}
                {!isTrackPlaying && (
                  <Play className="size-3.5 hidden group-hover:inline fill-current text-foreground" />
                )}
              </span>

              <img
                src={track.coverUrl}
                alt={track.album}
                className="size-9 rounded object-cover flex-shrink-0"
              />

              <div className="flex flex-col flex-1 min-w-0">
                <span className={cn("text-sm font-medium truncate", isCurrentTrack ? "text-primary" : "text-foreground")}>
                  {track.title}
                </span>
                <span className="text-xs text-muted-foreground truncate">
                  {track.artist}
                </span>
              </div>

              <span className="hidden sm:block w-40 text-xs text-muted-foreground truncate">
                {track.album}
              </span>

              <button
                onClick={(e) => {
                  e.stopPropagation()
                  actions.toggleLike(track.id)
                }}
                className={cn(
                  "p-1 opacity-0 group-hover:opacity-100 transition-opacity",
                  player.isLiked[track.id] && "opacity-100 text-primary"
                )}
                aria-label="Like"
              >
                <Heart
                  className={cn(
                    "size-3.5",
                    player.isLiked[track.id] ? "fill-primary text-primary" : "text-muted-foreground"
                  )}
                />
              </button>

              <span className="text-xs text-muted-foreground tabular-nums">
                {formatDuration(track.duration)}
              </span>
            </button>
          )
        })}
      </div>
    </div>
  )
}
